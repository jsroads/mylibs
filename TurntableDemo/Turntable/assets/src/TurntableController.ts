import {_decorator, Component, Node, Prefab, instantiate, Button, Color, v3, Label, EditBox, Sprite} from "cc";

const {ccclass, property} = _decorator;

const GRID_SIZE: number = 80; // 转盘格子尺寸
const TURNTABLE_SIZE: number = 5; // 横向纵向格子数
@ccclass("TurntableController")
export class TurntableController extends Component {

    @property({type: Prefab})
    public itemPrefab: Prefab = null; // 转盘格子预制体
    ...
    private itemList: Node[] = []; // 转盘格子节点列表
    ...
    private currentBg: Sprite = null;// 前一个显示对象
    ...
    private initTurntable() {
        ...
        for (let i = 0; i < TURNTABLE_SIZE * TURNTABLE_SIZE; i++) {
            const itemNode = instantiate(this.itemPrefab); // 实例化预制体
            // 设置节点位置
            itemNode.position = v3(
            ...
            if (array[i]) {
                itemNode.getChildByName("num").getComponent(Label).string = array[i] + "";
                itemNode.name = array[i] + "";
                this.itemList.push(itemNode);
            } else {
                itemNode.getChildByName("num").getComponent(Label).string = "";
            }
        }
        // 对 itemList 按数字排序
    ...
    private resetNumbers() {
        ...
        this.itemList.forEach((value, index) => {
            const bg = value.getChildByName("bg").getComponent(Sprite);
            bg.color = Color.WHITE;
            // 设置数字节点的文本为对应的数字
            value.getChildByName("num").getComponent(Label).string = value.name;
        });
    }
    ...
    private updateTurntable() {
        // 重置上一个数字颜色
        if (this.currentBg) this.currentBg.color = Color.WHITE;
        // 点亮当前数字
        this.highlightNumber(this.currentNumberIndex, Color.RED);
...
// 停止转盘
    private stopTurntable() {
        this.highlightNumber(this.targetNumberIndex, Color.GREEN); // 点亮目标数字
        this.setAllNumbersTo100(); // 将所有数字设置为100
        this.button.interactable = true;
    }

// 点亮指定数字
    private highlightNumber(index: number, color: Color) {
        const numberNode = this.itemList[index];
        if (numberNode) {
            const bg = numberNode.getChildByName("bg").getComponent(Sprite);
            bg.color = color;
            this.currentBg = bg;
        }
    }
...
// 计算目标数字索引
    ...
    private setAllNumbersTo100() {
        for (const numberNode of this.itemList) {
            // 设置数字节点的文本为100
            numberNode.getChildByName("num").getComponent(Label).string = "100";
        }
    }
