/**
 * Created by jsroads on 2022/11/25.14:35 下午
 * Note:
 */
const { ccclass, property } = cc._decorator;
const NUMBER_OF_GATHERED_TOUCHES_FOR_MOVE_SPEED = 2;//统计最大移动次数个数
@ccclass("MoveTouchNode")
export class MoveTouchNode {
    @property({
        visible: true,
        type: cc.Node
    })
...
@ccclass
export class DoubleMapTouchController extends cc.Component {
    @property({
        tooltip: '目标节点',
        type: cc.Node
    })
    ...
    @property({
        ...
        range: [0.01, 1],
        slide: true,
        visible: function (this: QuicksDoubleMapTouchController) {
            return this.inertia;
        }
    })
    ...
    public nodeList: MoveTouchNode[] = []; //阻尼系数，摩擦力
    ...
    protected update(dt: number) {
        ...
        //地图比较小 就不能再使用惯性
        if(this.realMaxScale>= this.map.scale){
            return;
        }
    ...
    //计算距离
    private static moveDistance(touch: cc.Touch): cc.Vec3 {
        ...
        this.node.on(cc.Node.EventType.TOUCH_MOVE, (event: cc.Event.EventTouch) => {
            ...
            if (touches.length >= 2) {
                ...
                if (this.isMoving || this.canStartMove(touch)) {
                    this.isMoving = true;
                    const distance: cc.Vec3 = QuicksDoubleMapTouchController.moveDistance(touch);
    ...
    private smoothOperate(target: cc.Node, pos: cc.Vec3, scale: number): void {
        ...
        this.node.emit("touch_move_distance", this.map.position.clone(),this.map.scale);
        // render ui
    ...
    private dealMove(dir: cc.Vec3, map: cc.Node, container: cc.Node): void {
        ...
        this.node.emit("touch_move_distance", targetPos,map.scale);
    ...
    protected onDisable(): void {
        // console.log("QuicksDoubleMapTouchController===>");
        this.map.scale = this.defaultScaling;
    }
}