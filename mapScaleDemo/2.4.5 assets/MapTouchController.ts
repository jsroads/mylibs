...
@ccclass
export class MapTouchBetterController extends cc.Component {
    @property({
        type: cc.Node,
        tooltip: '目标节点'
    })
    ...
    private deltaVec2 = new cc.Vec2(0, 0);
    ...
    private canStartMove(touch: cc.Touch): boolean {
        let startPos: cc.Vec2 = this.deltaVec2;
    ...
    // 有些设备单点过于灵敏，单点操作会触发TOUCH_MOVE回调，在这里作误差值判断
    private moveDistance(touch: cc.Touch): cc.Vec2 {
        return cc.v2(touch.getLocation().x - this.deltaVec2.x, touch.getLocation().y - this.deltaVec2.y);
    }
        ...
        this.node.on(cc.Node.EventType.TOUCH_MOVE, (event: cc.Event.EventTouch) => {
            ...
            if (touches.length >= 2) {
                ...
                if (this.isMoving || this.canStartMove(touch)) {
                    ...
                    let dir: cc.Vec3 = cc.v3(distance.x, distance.y);
