...
@ccclass
export class OriginMapController extends cc.Component {
    ...
    public moveOffset: number = 5;

    @property({
        tooltip: '滚轮缩放比率'
    })
    public increaseRate: number = 3000;
    ...
    private smoothOperate(target: cc.Node, pos: cc.Vec2, scale: number): void {
        ...
        if (this.minScale <= scale && scale <= this.maxScale) {
            ...
            let tempPos:cc.Vec3 = target.position.sub(cc.v3(gapPos.x,gapPos.y,0));
            let mapPos: cc.Vec2 = cc.v2(tempPos.x,tempPos.y);
    ...
    private dealScalePos(pos: cc.Vec2, target: cc.Node): void {
        ...
        target.position = cc.v3(pos.x,pos.y,0);
    }
