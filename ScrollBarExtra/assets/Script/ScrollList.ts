import { LayerType, ScrollListItem } from "./ScrollListItem";
 ...
 * @doc 列表组件.
 * @end
 * @see https://github.com/gh-kL/cocoscreator-list
 * 论坛地址:https://forum.cocos.org/t/topic/79055
 ******************************************/
...
export class ScrollList extends cc.Component {
	@property({ type: cc.Enum(LayerType), tooltip: "是否分层渲染" })
	...
	@property({
		...
		tooltip: CC_DEV && "初始化动画事件",
	})
	public animationEvent: cc.Component.EventHandler = new cc.Component.EventHandler();
	//模板类型
	...
	private _mainPool: cc.NodePool = null!;
	private _poolList: cc.NodePool[] = [];
	...
	/*分层子节点数组*/
	private _contentList: cc.Node[] = [];

	private _showAnimation: boolean = false;
	public get showAnimation(): boolean {
		return this._showAnimation;
	}

	public set showAnimation(value: boolean) {
	...
	set numItems(val: number) {
		...
		if (!t.checkInited(false)) {
			// cc.error("numItems set the wrong::请在start()周期或者后面时候调用", val);
			return;
		}
		if (val == null || val < 0) {
	...
	onDestroy() {
		...
		this._mainPool.clear();
		this._poolList.forEach((value) => {
			value.clear();
		});
	}
	...
	_init() {
		...
		if (!t._layout) {
			console.error("滑动列表需要添加 cc.Layout 组件");
			return;
		}
		...
		t._inited = true;
		//初始化 分层
		t._mainPool = new cc.NodePool();
		const tmpNode = t.templateType === TemplateType.PREFAB ? cc.instantiate(t.tmpPrefab) : t.tmpNode;

		if (!CC_EDITOR && cc.isValid(tmpNode)) {
			let listItem = tmpNode.getComponent(ScrollListItem);
			this.contentLayerType = listItem ? listItem.contentLayerType : LayerType.NONE;
			if (this.contentLayerType === LayerType.SLICE) {
				for (let i = 0; i < listItem.layerNodeList.length; i++) {
					let childContent = new cc.Node();
					childContent.anchorX = this.content.anchorX;
					childContent.anchorY = this.content.anchorY;
					childContent.position = this.content.position;
					childContent.parent = this.content.parent;
					childContent.scaleX = this.content.scaleX;
					childContent.scaleY = this.content.scaleY;
					t._poolList.push(new cc.NodePool()); //这是个池子..
	...
	setTemplateItem(item: any) {
		...
		let com = item.getComponent(ScrollListItem);
		let remove = false;
		if (!com) remove = true;
		// if (com) {
	...
	_createOrUpdateItem(data: any, animate: boolean) {
		...
		if (!item) {
			...
			const vec3Point: cc.Vec3 = new cc.Vec3(data.x, data.y, 0);
		...
		if (animate && this.animationEvent) {
			cc.Component.EventHandler.emitEvents([this.animationEvent], item, data.id % this._actualNumItems);
		}
	}
	 ...
	 * @returns
	 */
	getItemByListId(listId: number, content?: cc.Node) {
		content = content || this.content;
		if (content) {
			for (let n: number = content.childrenCount - 1; n >= 0; n--) {
	 ...
	 * @returns
	 */
	_getOutsideItem(content?: cc.Node) {
		content = content || this.content;
		let item: any;
		let result: any[] = [];
		for (let n: number = content.childrenCount - 1; n >= 0; n--) {
