...
export function readU64(b: Uint8Array | number[], n: number): number {
    let x = 0;
    x |= b[n++] << 0;
    x |= b[n++] << 8;
    x |= b[n++] << 16;
...
export function readU32(b:Uint8Array | number[], n: number): number {
    let x = 0;
    x |= b[n++] << 0;
    x |= b[n++] << 8;
    x |= b[n++] << 16;
...
export function writeU32(b: Uint8Array | number[], n: number, x: number): void {
    b[n++] = (x >> 0) & 0xff;
    b[n++] = (x >> 8) & 0xff;
    b[n++] = (x >> 16) & 0xff;
    b[n++] = (x >> 24) & 0xff;
